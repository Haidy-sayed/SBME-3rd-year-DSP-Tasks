...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.label_2.setGeometry(QtCore.QRect(60, 20, 55, 16))
        self.label_2.setObjectName("label_2")
        self.label_3 = QtWidgets.QLabel(self.centralwidget)
        self.label_3.setGeometry(QtCore.QRect(160, 20, 55, 16))
        self.label_3.setObjectName("label_3")
        self.label_4 = QtWidgets.QLabel(self.centralwidget)
        self.label_4.setGeometry(QtCore.QRect(260, 20, 55, 16))
  ...
  #      self.max_x_limit=100
        ...
        self.scrollFinal=5
        self.p1.setXRange(self.scrollInitial, 10, padding=0)     
        ...
        #setting trivial initial data
        self.curve1.setData([0],[0])
        self.curve2.setData([0],[0])
        self.curve3.setData([0],[0])
        ...
        self.horizontalScrollBar.setGeometry(QtCore.QRect(0, 347, 1271, 14))
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.slowerButtonCh.clicked.connect(lambda: self.slow(self.channel))
        self.fasterButtonCh.clicked.connect(lambda: self.fast(self.channel))
        ...
        self.addLabelButton.clicked.connect(lambda: self.addTitle(self.channel))
    ...
    def open_file2(self):
      """opens a file from the brower """
      file_path=QFileDialog.getOpenFileName()
      self.file_name2=file_path[0].split('/')[-1]
      self.read_data2(self.file_name2)

    def open_file3(self):
      """opens a file from the brower """
      file_path=QFileDialog.getOpenFileName()
      self.file_name3=file_path[0].split('/')[-1]
      self.read_data3(self.file_name3)
    ...
    def read_data2(self,file_name2):
        """loads the data from chosen file"""  
        df2=pd.read_csv(file_name2)
        global time2
        global amp2       
        self.label2=file_name2  #trial
    ...
    def read_data3(self,file_name3):
        """loads the data from chosen file"""
        df3=pd.read_csv(file_name3)
        global time3
        global amp3  
        self.label3 = file_name3
    ...
    def draw(self,time,amp,color):
        ...
        self.curve1.setData(self.time1[0:self.index+100], self.amp1[0:self.index+100], pen=pen)     
        self.timer1.setInterval(self.timerInterval1)
    ...
    def draw2(self,time,amp,color):
        ...
        self.index2=0  
        pen = pyqtgraph.mkPen(color) #signal color
        self.curve2.setData(self.time2[0:self.index2+100], self.amp2[0:self.index2+100], pen=pen)
        self.timer1.setInterval(self.timerInterval1)
        self.timer1.timeout.connect(lambda:self.update_plot_data2(self.time2,self.amp2))
        self.timer1.start()
    ...
    def draw3(self,time,amp,color):
        ...
        self.index3=0  
        pen = pyqtgraph.mkPen(color) #signal color
        self.curve3.setData(self.time3[0:self.index3+100], self.amp3[0:self.index3+100], pen=pen)
        self.timer1.setInterval(self.timerInterval1)
        self.timer1.timeout.connect(lambda:self.update_plot_data3(self.time3,self.amp3))
        self.timer1.start()                     

    def last_plotted(self):
        """determines last plotted value on x-axis to set our range later"""
        last_t1=self.curve1.getData()[0][-1]
        last_t2=self.curve2.getData()[0][-1]
        last_t3=self.curve3.getData()[0][-1]
        max_t=max(last_t1,last_t2,last_t3)
        return max_t
    
    def update_plot_data(self,time,amp): 
        """updates the data plotted on graph to get dynamic signal"""
        
        dynamic_time = time[0:self.index+100]  
        dynamic_amp = amp[0:self.index+100]
        max_t=self.last_plotted()
        
        self.p1.setXRange(max_t-10, max_t, padding=0) 
        
        self.index=self.index+100
        if self.index+100>len(time):
            self.index=0
    ...
    def update_plot_data2(self,time2,amp2): 
        ...
        dynamic_time2 = time2[0:self.index2+100]  
        dynamic_amp2 = amp2[0:self.index2+100]
        max_t=self.last_plotted()
        
        self.p1.setXRange(max_t-10, max_t, padding=0)       

        self.index2=self.index2+100
        if self.index2+100>len(time2):
            self.index2=0
    ...
    def update_plot_data3(self,time3,amp3): 
        ...
        dynamic_time3 = self.time3[0:self.index3+100]  
        dynamic_amp3 = self.amp3[0:self.index3+100]
        max_t=self.last_plotted()
        ...
        self.index3=self.index3+100
        if self.index3+100>len(time3):
            self.index3=0
    ...
    def addTitle(self,channel):
        if channel=="Ch1":

            self.label_2.setText(self.file_name)
            self.label_2.update()
        elif channel=="Ch2":
            self.label_3.setText(self.file_name2)
            self.label_3.update()
        else:
            self.label_4.setText(self.file_name3)
            self.label_4.update()
    ...
    def slow(self,channel):
        self.timerInterval1 = self.timerInterval1 * 2 
        self.timer1.setInterval(self.timerInterval1 )
           
        

    def fast(self,channel):
        self.timerInterval1 = self.timerInterval1 / 2 
        self.timer1.setInterval(self.timerInterval1 )
    ...
    def spectro(self , channel):
        if channel =="Ch1":
            if not self.amp1:
                ...
                fig=plt.figure()
                self.spec_gram = plt.specgram(self.amp1, Fs=200 ,cmap=self.palette)
